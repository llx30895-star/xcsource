...
# -*- coding: utf-8 -*-
"""
产品图片识别和表格整理脚本（带 AI 视觉识别 + 自动转移）
使用 Claude Vision API 识别产品图片，生成提示词和中英文描述
...
BRAND_PREFIXES = ["iKKLE Home_", "MICEPLAY_", "SOUL HAND_"]
...
def identify_product_with_ai(image_path):
    """使用 Claude Vision API 识别产品"""
    if not API_KEY:
        log("  警告: 未设置 ANTHROPIC_API_KEY，使用占位符")
        return {
            "prompt": "未配置 API",
            "cn_desc": "未配置 API",
...
def main():
    if not API_KEY:
        log("警告: 未设置 ANTHROPIC_API_KEY 环境变量")
        log("将创建表格但不进行 AI 识别")
